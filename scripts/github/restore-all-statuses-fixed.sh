...
# Get all issues
echo "📊 Issue一覧を取得中..."
ALL_ISSUES=$(gh api graphql -f query='query { repository(owner: "kencom2400", name: "account-book") { issues(first: 100, states: [OPEN, CLOSED]) { nodes { number } } } }' | jq -r '.data.repository.issues.nodes[].number')

...
DONE_OPTION_ID=$(echo "$FIELD_INFO" | jq -r '.options[] | select(.name == "✅ Done") | .id')

echo "   フィールドID: $FIELD_ID"
echo "   Done オプションID: $DONE_OPTION_ID"

# ステータスを変更
echo ""
echo "🔄 ステータスを 'Done' に変更中..."
...
echo ""
echo "✅ Issue #${ISSUE_NUMBER} のステータスを 'Done' に変更しました！"
echo ""

...
- `any(. == "In Progress")`: 配列内に特定の要素が存在するかをチェック（意図が明確）
- `contains(["In Progress"])`: 部分配列が含まれるかをチェック（複数要素の確認に適している）
- 単一要素の存在確認には `any()` を使用することで、コードの意図が明確になる

#### 使い分けガイドライン

| ユースケース         | 推奨フィルター             | 例                              |
| -------------------- | -------------------------- | ------------------------------- |
| 単一要素の存在確認   | `any(. == "値")`           | `any(. == "In Progress")`       |
| 複数要素の存在確認   | `contains(["値1", "値2"])` | `contains(["bug", "critical"])` |
| すべての要素がマッチ | `all(条件)`                | `all(. != "done")`              |

**Geminiレビューから学んだ教訓**:

- コマンドラインツールでも可読性は重要
- 適切な関数を選択することで、コードレビュアーが意図を理解しやすくなる
- ドキュメント化により、将来の保守性が向上する
...
**原則**: ドキュメントファイルでは、冗長な表現を避け、簡潔で明確な記述を心がける。
...
`.cursor/rules/project.md` を読み込んでください。
...
```markdown
### 1. `.cursor/rules/project.md` - プロジェクト全体のガイドライン
...
- ドキュメントファイルのフォーマット統一が重要
- 冗長な表現は避け、簡潔な記述を心がける
- 見出しにファイルパスを含めることで情報密度を高める

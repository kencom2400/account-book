...
- [ ] クラス図へのリンクが設定されている（作成中）
- [ ] シーケンス図へのリンクが設定されている（作成中）
- [ ] 入出力設計へのリンクが設定されている（作成中）
...
- [ ] 画面遷移図が作成されている（認証方式に応じた入力フォームの切り替え）
...
- [ ] クラス図の作成（認証タイプの列挙型、BankCredentials型の拡張、バリデーションロジック）
- [ ] シーケンス図の作成（認証方式に応じた処理フロー、接続テストフロー）
- [ ] 入出力設計の作成（API仕様、DTOの拡張、リクエスト/レスポンス例）
- [ ] 画面遷移図の作成（認証方式に応じた入力フォームの切り替え）

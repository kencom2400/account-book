...
- **挙動**:
  ...
  - クリック時: セレクトボックスと保存/キャンセルボタンに切り替え
  - 選択後、保存ボタンをクリックすると更新処理を実行
...
- [ ] セレクトボックスにはすべてのカテゴリが表示される
  - **注記**: カテゴリタイプの変更も可能（例: 誤って「支出」に分類された「収入」を修正可能）
  - Backend APIでカテゴリタイプの変更も記録される

...
**イベントカテゴリと取引カテゴリの関連マッピング**:

| イベントカテゴリ              | 関連する取引カテゴリ例         |
| ----------------------------- | ------------------------------ |
| `TRAVEL` (旅行)               | 交通費、宿泊費、飲食費、娯楽費 |
| `EDUCATION` (就学)            | 教育費、書籍費、文具費         |
| `PURCHASE` (高額購入)         | 家具・家電、自動車、住宅関連   |
| `MEDICAL` (医療)              | 医療費、薬代、健康診断費       |
| `LIFE_EVENT` (ライフイベント) | 結婚式、出産、引越し関連費     |
| `INVESTMENT` (投資)           | 投資関連、証券関連             |
| `OTHER` (その他)              | すべてのカテゴリ（関連性なし） |

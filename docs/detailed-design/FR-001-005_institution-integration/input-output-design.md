...
- `details`フィールドは配列形式で、複数のエラーを同時に返すことができます
- `details`フィールドは任意（`details`が存在しない場合もあります）
- 各エラー詳細には`field`（任意）と`message`（必須）が含まれます
- `field`はエラーが発生したフィールド名を表します（バリデーションエラーの場合）

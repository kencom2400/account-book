...
**簡単な機能説明**: クレジットカードの月別集計額（FR-012で算出）と銀行口座からの実際の引落額を自動的に照合し、金額・日付・摘要の3つの条件でマッチング判定を行います。完全一致の場合は照合ステータスを`MATCHED`に更新し、不一致の場合はアラートを生成します。引落予定日の3日後（自動）またはユーザーによる手動実行で照合を実行します。
...
**概要**: 指定されたカードと請求月の月別集計データを取得し、引落予定日前後3営業日の銀行取引を検索して、金額・日付・摘要の3つの条件でマッチング判定を行います。完全一致の場合は照合ステータスを`MATCHED`に更新し、不一致の場合はアラートを生成します。
...
| RC002        | 外部サービスへの接続失敗       | 502/503        | 銀行連携を確認       |

...
- 月末締め（締め日: `0`または`31`）
  - 各月の最終日を締め日とする（1月31日、2月28/29日、3月31日...）
- 15日締め（締め日: `15`）
  - 例: 15日締め、翌月10日払い
- 10日締め（締め日: `10`）
  - 例: 10日締め、翌月5日払い
...
4. カテゴリ別に利用額を集計
5. ✅ **Phase 1**: 合計金額のみを算出
6. ⏳ **Phase 2**（FR-013）: ポイント利用・キャッシュバックを控除し、最終支払額を算出
...
5. **締め日計算**
   ...
   - 12月から1月の遷移処理
   - **閏年対応（初期実装で対応）**
     - 2月の締め日計算は閏年を考慮（Date APIが自動的に処理）
     - BillingPeriodCalculatorで`new Date(year, 2, 0).getDate()`で2月の最終日を取得

6. **金融計算の精度**
   - **金額は整数（円単位）で扱う**
   - JavaScriptのnumber型の浮動小数点誤差を避けるため、少数以下を扱わない設計
   - 将来的に銭単位が必要な場合は、Decimal.jsライブラリの導入を検討

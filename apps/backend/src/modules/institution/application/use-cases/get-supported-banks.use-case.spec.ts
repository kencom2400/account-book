...
describe('GetSupportedBanksUseCase', () => {
  ...
  describe('execute', () => {
    it('should return all supported banks when no query is provided', async () => {
      const banks = await useCase.execute();
    ...
    it('should return banks with correct structure', async () => {
      const banks = await useCase.execute();
      const bank = banks[0];
    ...
    it('should filter banks by category (MEGA_BANK)', async () => {
      const banks = await useCase.execute({
    ...
    it('should filter banks by category (ONLINE_BANK)', async () => {
      const banks = await useCase.execute({
    ...
    it('should filter banks by category (REGIONAL_BANK)', async () => {
      const banks = await useCase.execute({
    ...
    it('should search banks by name (case insensitive)', async () => {
      const banksUpper = await useCase.execute({
        searchTerm: 'UFJ',
      });
      const banksLower = await useCase.execute({
        searchTerm: 'ufj',
      });
    ...
    it('should search banks by bank code', async () => {
      const banks = await useCase.execute({
        searchTerm: '0005',
      });
    ...
    it('should include major banks in the list', async () => {
      const banks = await useCase.execute();
    ...
    it('should include online banks in the list', async () => {
      const banks = await useCase.execute();
      const bankNames = banks.map((bank) => bank.name);
    ...
    it('should have unique bank IDs', async () => {
      const banks = await useCase.execute();
  ...
  describe('findByCode', () => {
    it('should find bank by code', async () => {
      const bank = await useCase.findByCode('0005');
    ...
    it('should return null for non-existent code', async () => {
      const bank = await useCase.findByCode('9999');

      expect(bank).toBeNull();
    });

    it('should return null for unsupported bank', async () => {
    ...
    it('should find test bank', async () => {
      const bank = await useCase.findByCode('0000');

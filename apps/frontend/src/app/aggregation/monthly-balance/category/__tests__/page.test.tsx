...
const mockRouter = {
  ...
  prefetch: jest.fn(),
  back: jest.fn(),
  pathname: '/aggregation/monthly-balance/category',
  query: {},
  asPath: '/aggregation/monthly-balance/category',
};
...
describe('CategoryBreakdownPage', () => {
  // テスト全体のタイムアウトを30秒に設定
  ...
  it('ローディング状態を表示する', () => {
    ...
    // Act: コンポーネントをレンダリング
    render(<CategoryBreakdownContent />);
  ...
  it('カテゴリ別内訳のコンテンツを表示する', async () => {
    ...
    await waitFor(
      () => {
        expect(screen.getAllByText('カテゴリ別内訳').length).toBeGreaterThan(0);
  ...
  it('支出カテゴリの内訳を表示する', async () => {
    // Act: コンポーネントをレンダリング
    render(<CategoryBreakdownContent />);

    // Assert: 支出カテゴリの内訳が表示される
    await waitFor(
      () => {
  ...
  it('収入ボタンをクリックすると収入カテゴリを表示する', async () => {
    // Act: コンポーネントをレンダリング
    const user = userEvent.setup({ delay: null });
    render(<CategoryBreakdownContent />);

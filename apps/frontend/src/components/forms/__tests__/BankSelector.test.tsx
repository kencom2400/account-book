...
describe('BankSelector', () => {
  ...
  it('should display category tabs', async () => {
    ...
    await waitFor(() => {
      ...
      const megaBankButtons = screen.getAllByRole('button', { name: /メガバンク/ });
      expect(megaBankButtons.length).toBeGreaterThan(0);
    });
  });
  ...
  it('should filter banks by category', async () => {
    ...
    // メガバンクタブをクリック（複数ある場合は最初のものを使う）
    const megaBankButtons = screen.getAllByRole('button', { name: /メガバンク/ });
    fireEvent.click(megaBankButtons[0]);
  ...
  it('should display bank category with each bank', async () => {
    ...
    await waitFor(() => {
      // 複数の銀行カテゴリが表示されることを確認
      const allText = document.body.textContent || '';
      expect(allText).toContain('ネット銀行');
      expect(allText).toContain('地方銀行');
    });
  });
});
